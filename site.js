  ...
  function renderEducation() {
    ...
    const content = items.map((item, index) => {
      const titleHtml = item.url
        ...
        : esc(item.title);
      return `
        <section class="content-section research-section" id="education-${index}">
          <header class="people-block-heading" aria-label="${esc(item.title)}">
            <span>${String(index + 1).padStart(2, "0")}</span>
            <h2>${titleHtml}</h2>
          </header>
        </section>
    ...
    return renderTocLayout(tocItems, content);
  }

...
function renderSrt() {
  ...
  return `<section id="education-srt" class="srt-section indexed-section" lang="en">
    <header class="srt-intro"><p class="srt-eyebrow">UNDERGRADUATE RESEARCH &middot; SRT 2026&ndash;2027</p>
      <p class="srt-back"><a href="${href("education")}">&larr; Education</a></p><h1>${esc(selectedProject.title)}</h1><p>${esc(selectedProject.question)}</p><p>A proposed Student Research Training (SRT) project for Tsinghua undergraduates, autumn 2026&ndash;2027.</p>
      ...
      <div class="srt-callout"><strong>Student applications: 15&ndash;25 October 2026 (provisional)</strong><p>14&ndash;27 September is the project proposal window. Project approval, available places, eligibility, and final dates are subject to the SRT system and official university notices.</p><a href="#srt-apply">How to apply &darr;</a></div>
    </header>
    ${list([selectedProject], (project) => `<article class="srt-project"><h2>Explore the project</h2>
      ...
      <details><summary>Research schedule and budget</summary><ul>${list(project.plan, item => `<li>${esc(item)}</li>`)}</ul><p>${esc(project.budget)}</p><p>These are requested project budgets, subject to approval, rather than individual student grants.</p></details>
    </article>`)}
    <section id="srt-apply" class="srt-apply indexed-section"><h2>How to apply</h2><p>SRT is Tsinghua University's Student Research Training program. Interested undergraduates can contact the supervisor to discuss the projects. During the student application window, log in to the Tsinghua information portal, check approved projects and eligibility in the SRT system, and submit your application.</p>
      ...
      <h3>Autumn 2026: provisional timeline</h3><table><thead><tr><th scope="col">Stage</th><th scope="col">Dates (2026)</th></tr></thead><tbody>${list(srt.dates, ([label, date]) => `<tr${label === "Student applications" ? ' class="srt-registration-row"' : ""}><th scope="row">${esc(label)}</th><td>${esc(date)}</td></tr>`)}</tbody></table>
      <details><summary>Proposals, funding, and grades</summary><p>There is one proposal round this semester. Each faculty member may propose up to two projects and each student may propose one. Each project may accept up to five students. Supervisors should review student-led proposals promptly after submission.</p><p>Funding is capped at RMB 5,000 per project. Projects that do not request funding will not receive it. SRT funds cannot be used for work-study payments. Funding for projects starting in 2026-2027 will be disbursed in the spring semester of that academic year.</p><p>Supervisors may submit grades when projects are completed; grades normally cannot be changed after submission. Grades are generally transferred to transcripts at mid-semester and the end of the spring and autumn semesters, with no action required from students or supervisors. Check your grade promptly. Any appeal must be submitted to the supervisor's department within the prescribed appeal period and follow the university's grade review procedure.</p></details>
      <p class="srt-source">Source: the university's practical teaching office notice dated 14 September 2026. Official notices take precedence over the provisional dates above. Administrative enquiries: +86 10 62785589; jwcsjk (at) mailoa.tsinghua.edu.cn.</p>
    </section></section>`;
}
